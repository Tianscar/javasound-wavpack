...
class WordsUtils
{
    ...
    // them back to 32-bit unsigned values and store them. If length is not
    // exactly correct then we flag and return an error
    static int read_entropy_vars_encode(WavpackStream wps, WavpackMetadata wpmd)
    {
        byte[] byteptr = wpmd.data;
    ...
    // stuff is not implemented yet, so return an error if we get more data than
    // we know what to do with.
    static int read_hybrid_profile_encode(WavpackStream wps, WavpackMetadata wpmd)
    {
        byte[] byteptr = wpmd.data;
        int byte_idx = 0;
    ...
    static void write_entropy_vars(WavpackStream wps, WavpackMetadata wpmd)
    {
        ...
        read_entropy_vars_encode(wps, wpmd);
    }
    ...
    static void write_hybrid_profile(WavpackStream wps, WavpackMetadata wpmd)
    {
        ...
        wpmd.byte_length = byte_idx;
        read_hybrid_profile_encode(wps, wpmd);
    }
